/**
 * @file CLIHandler.cpp
 * @brief Implementation of the CLIHandler class for command-line interface management.
 *
 * This file provides the concrete implementation of the command-line interface system,
 * handling user authentication flows, admin dashboard interactions, and level management
 * through text-based menus. The CLI serves as the primary interface for administrative
 * functions and user authentication in the dual-mode application.
 */
...
namespace Gameplay
{
    ...
    CLIHandler::AdminDashboardResult CLIHandler::runAdminDashboard(const Engine::User &user)
    {
        ...
        if (user.role != Engine::UserRole::Admin)
            return result;
        ...
        if (choice == 1)
        {
            ...
            std::string levelsDir = "build/assets/levels/";
            if (std::filesystem::exists(levelsDir))
            {
                ...
                for (const auto &entry : std::filesystem::directory_iterator(levelsDir))
                {
                    if (entry.is_regular_file() && entry.path().extension() == ".json")
                    {
                        ...
                        if (filename != "level1.json" && filename != "level2.json")
                        {
            ...
            {
                std::cout << " - (Levels directory not found)" << std::endl;
            }
