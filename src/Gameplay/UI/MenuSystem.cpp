    ...
    namespace UI
    {
        int MenuSystem::updateMainMenu(const Common::InputState& input, std::vector<Common::RenderCommand>& commands)
        {
            ...
            if (input.mouseLeftDown && !wasMousePressed) {
                ...
                if (isButtonClicked(input.mouseX, input.mouseY, bx1, by1, bw, bh)) return 1;
                if (isButtonClicked(input.mouseX, input.mouseY, bx2, by2, bw, bh)) return 2;
            }
        ...
        int MenuSystem::updatePauseMenu(const Common::InputState& input, std::vector<Common::RenderCommand>& commands)
        {
            ...
            if (input.mouseLeftDown && !wasMousePressed) {
                wasMousePressed = true;
                if (isButtonClicked(input.mouseX, input.mouseY, bx, by1, bw, bh)) return 1; // Resume
                if (isButtonClicked(input.mouseX, input.mouseY, bx, by2, bw, bh)) return 2; // Main Menu
                if (isButtonClicked(input.mouseX, input.mouseY, bx, by3, bw, bh)) return 3; // Exit
            }
